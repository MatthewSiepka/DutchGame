...
from ui.game_old.game_scene_model import GameSceneModel, GameSelectionType
...
class GameSceneView(SceneView):
    ...
    def render_card_deck(self, height_of_deck, player: PlayerInfo, witch_one: int):
        ...
        for i in range(player.cards):
            ...
            render_card(self.stdscr, height_of_deck + 3, cards_start + card_length * i, card, color)
    ...
    def render_utility_card_row(self, height):
        ...
        self.stdscr.addstr(height, y_and_x[1] // 2 - 12 - len(stack_card_label) // 2, stack_card_label,
                           curses.color_pair(3))
        render_card(self.stdscr, height + 1, y_and_x[1] // 2 - 5 // 2 - 12, None, curses.color_pair(3))
        # card selected
        ...
        render_card(self.stdscr, height + 1, y_and_x[1] // 2 - 5 // 2, selected_card, curses.color_pair(3))
        ...
        self.stdscr.addstr(height, y_and_x[1] // 2 + 12 - len(used_stack_label) // 2, used_stack_label,
                           curses.color_pair(3))
        render_card(self.stdscr, height + 1, y_and_x[1] // 2 - 5 // 2 + 12, used_stack_card, curses.color_pair(3))
